...
class Loader
{
    ...
    public function boot()
    {
        ...
        add_action('after_setup_theme', [$this, 'load_textdomain'], 9999);
        add_action('init', [$this, 'trigger_fields_register'], 0);
        add_action('rest_api_init', [$this, 'initialize_widgets']);
        add_action('carbon_fields_fields_registered', [$this, 'initialize_containers']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_media_browser'], 0);
        add_action('admin_print_footer_scripts', [$this, 'enqueue_assets'], 9);
        add_action('admin_print_footer_scripts', [$this, 'initialize_ui'], 9999);
        add_action('edit_form_after_title', [$this, 'add_carbon_fields_meta_box_contexts']);
        add_action('wp_ajax_carbon_fields_fetch_association_options', [$this, 'fetch_association_options']);
    ...
    public function enqueue_assets()
    {
        ...
        $this->enqueue_script('vendor', ['wp-polyfill', 'jquery']);
        $this->enqueue_script('core', ['carbon-fields-vendor']);
        $this->enqueue_script('metaboxes', ['carbon-fields-vendor', 'carbon-fields-core']);

        if ($this->get_assets_context() === 'gutenberg') {
            $this->enqueue_style('blocks');
            $this->enqueue_script('blocks', ['carbon-fields-vendor', 'carbon-fields-core']);
        }
        ...
        wp_localize_script(
            ...
            'cf',
            apply_filters('carbon_fields_config', [
                'config' => [
     ...
     * @return void
     */
    protected function enqueue_style($src, $deps = [])
    {
     ...
     * @return void
     */
    protected function enqueue_script($src, $deps = [])
    {
        $suffix = $this->get_assets_suffix();
        $context = $this->get_assets_context();
